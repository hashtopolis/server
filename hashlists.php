...
if(isset($_POST['action'])){
	switch($_POST['action']){
		case 'hashlistsecret':
			...
			if ($secret == 1) {
				$FACTORIES::getagentsFactory()->getDB()->query("DELETE FROM hashlistusers JOIN agents ON agents.id=hashlistusers.agent WHERE hashlistusers.hashlist=$hlist AND agents.trusted<$secret");
			}
			if (!$res) {
