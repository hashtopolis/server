...
if(isset($_GET['id'])){
	...
	foreach($res as $agent){
		$ans = $DB->query("SELECT IF(count(*)>0,1,0) as working, chunks.task as task FROM chunks INNER JOIN assignments ON assignments.task=chunks.task INNER JOIN agents ON agents.id=assignments.agent WHERE agents.id=".$agent['id']." AND GREATEST(dispatchtime, solvetime)>".(time() - $CONFIG->getVal('chunktimeout')));
		$line = $ans->fetch();
