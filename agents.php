...
if(isset($_GET['id'])){
	...
	foreach($res as $agent){
		$ans = $DB->query("SELECT IF(count(*)>0,1,0) as working FROM chunks INNER JOIN assignments ON assignments.task=chunks.task INNER JOIN agents ON agents.id=assignments.agent WHERE agents.id=".$agent['id']." ".max($agent['dispatchtime'], $agent['solvetime']).">".(time() - $CONFIG->getVal('chunktimeout')));
		$line = $ans->fetch();
