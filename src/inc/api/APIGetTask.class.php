...
class APIGetTask extends APIBasic {
  ...
  private function sendTask($task, $assignment) {
    ...
    $joined = $FACTORIES::getFileTaskFactory()->filter(array($FACTORIES::FILTER => $qF, $FACTORIES::JOIN => $jF));
    /** @var $files File[] */
    ...
    $this->sendResponse(array(
        ...
        PResponseGetTask::ATTACK_COMMAND => $task->getAttackCmd() . " --hash-type=" . $hashlist->getHashTypeId(),
