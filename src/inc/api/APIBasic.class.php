...
abstract class APIBasic {
  ...
  protected function getBestTask() {
    global $FACTORIES;
    
    // load all groups where this agent has access to
    $qF = new QueryFilter(AccessGroupAgent::AGENT_ID, $this->agent->getId(), "=");
    ...
    foreach ($taskWrappers as $taskWrapper) {
      ...
      foreach ($tasks as $task) {
        ...
        foreach ($files as $file) {
          if ($file->getIsSecret() > $this->agent->getIsTrusted()) {
            $permitted = false;
          }
        }
        if (!$permitted) {
        ...
        if ($task->getIsCpuTask() != $this->agent->getCpuOnly()) {
          continue;
        }
   ...
   */
  protected function checkChunkSplit($chunk) {
    global $FACTORIES;
   ...
   */
  protected function getImportantTask($task1, $task2) {
    global $FACTORIES;
   ...
   */
  protected function depriorizeAllTasks($taskWrapper, $hashlists){
    global $FACTORIES;
