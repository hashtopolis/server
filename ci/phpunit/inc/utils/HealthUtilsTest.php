...
final class HealthUtilsTest extends TestBase {
  ...
  public function testResetAgentCheckResetsCompletedAgentUnderPendingCheck(): void {
    HealthUtils::resetAgentCheck($this->completedAgent->getId());
    
    $updated = Factory::getHealthCheckAgentFactory()->get($this->completedAgent->getId());
    $this->assertSame(DHealthCheckAgentStatus::PENDING, $updated->getStatus());
    $this->assertSame(0, $updated->getCracked());
    $this->assertSame(0, $updated->getNumGpus());
    $this->assertSame(0, $updated->getStart());
    ...
    $parentCheck = Factory::getHealthCheckFactory()->get($this->healthCheck->getId());
    $this->assertSame(DHealthCheckStatus::PENDING, $parentCheck->getStatus());
  }
