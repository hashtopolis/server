...
def get_bearer_token():
    """Request an apiv2 JWT with the test credentials, for raw requests."""
    cfg = get_test_config()
    r = requests.post(f"{cfg['hashtopolis_uri']}/api/v2/auth/token",
...
def do_create_local_cracker(source_type='inline', source_data=None,
                            content=SEVEN_ZIP_MAGIC + b'local-cracker-test',
                            extra_payload={}, **kwargs):
    """Create a cracker binary by uploading an archive instead of providing a url.

    source_data defaults to the base64 encoded content for source_type 'inline'.
    For 'import' it is the filename of a file in the import directory (e.g. one
    uploaded with FileImport.do_upload), for 'url' a http/https url.
    """
    if source_data is None:
        source_data = base64.b64encode(content).decode()
    payload = json.loads(
        Path(__file__).parent.joinpath('testfiles/cracker/create_cracker_001.json').read_text('UTF-8'))
    # the download url is generated by the server when the archive is uploaded
    del payload['downloadUrl']
    final_payload = {**payload, **extra_payload,
                     'sourceType': source_type, 'sourceData': source_data}
    obj = Cracker(**final_payload)
    obj.save()
    return obj
...
class BaseTest(unittest.TestCase):
    ...
    def create_local_cracker(self, **kwargs):
        return self._create_test_object(do_create_local_cracker, **kwargs)
