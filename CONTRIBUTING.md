...
## Dependency pinning (MkDocs/docs build)

The docs build workflows (`.github/workflows/docs.yml` and `.github/workflows/docs-build.yml`) use pinned MkDocs dependencies:

- `doc/requirements.in` - the human-edited list of direct dependencies (mkdocs and the plugins enabled in `mkdocs.yml`). **Edit this file** to add, remove, or constrain a dependency. When adding a new plugin to `mkdocs.yml`, also add it here.
- `doc/requirements.txt` - the fully resolved, transitively pinned lock file (with hashes), generated by `pip-compile`. **Never hand-edit this file**; regenerate it instead.
