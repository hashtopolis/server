	\section*{Agents (\textit{agent})}
		...
		\subsection*{\textit{listAgents}}
			List all agents with some basic information.
			{
				\color{blue}
				\begin{verbatim}
				{
		...
		\subsection*{\textit{get}}
			Retrieve all the information about a specific agent by providing its ID. The last action time is a UNIX timestamp and if the configuration on the server is set to hide the IP of the agents, the value will just be \textit{Hidden} instead of the IP.
			{
				\color{blue}
				\begin{verbatim}
				{
